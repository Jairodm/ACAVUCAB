...
<head>
    ...
    <title>ACAVUCAB - Consulta de telefono</title>
...
<body>
    ...
    <div class="container">
        <h1 class="display-4 text-center">Telefono</h1>
        <hr class="bg-warning">


        <div class="container center_div">

            <form action="{{route ('editar.telefono', $telefono->codigo_telefono)}}" class="needs-validation" method="POST" novalidate>

                @method('PUT')
                @csrf
        
                        <div class="col-md-6 mb-3">
                            <label for="vencimiento">Número telefónico</label>
                            <input type="text" class="form-control" name="numeroTelefonico"  value="{{$telefono->codigo_area}}-{{$telefono->numero}}" placeholder="" required>
            ...
            <form action="{{route ('eliminar.telefono',     $telefono->codigo_telefono)}}" method="POST" class="d-inline">

                @method('DELETE')
                @csrf
                <button class="btn btn-warning btn-lg " style="background-color:red; color:white" type="submit">Eliminar telefono</button>
